...
public class Floater : MonoBehaviour {
    ...
    private void FixedUpdate() {
        ...
        submerged = transform.position.y < WaterLevelManager.Instance.GetWaterPlaneHeight() + waterHeight;

        if (submerged) {
            float displacementMulti =
                Mathf.Clamp01(
                    (WaterLevelManager.Instance.waterLevel + waterHeight - transform.position.y) / depthBefSub) *
                displacementAmt;

            rb.AddForceAtPosition(new Vector3(0f, Mathf.Abs(Physics.gravity.y) * displacementMulti, 0f),
                transform.position, ForceMode.Acceleration);

            rb.AddForce(-rb.linearVelocity * (displacementMulti * waterDrag * Time.fixedDeltaTime),
                ForceMode.VelocityChange);

            rb.AddTorque(-rb.angularVelocity * (displacementMulti * waterAngularDrag * Time.fixedDeltaTime),
                ForceMode.VelocityChange);
        }
    }
    ...
    private void OnDrawGizmos() {
        ...
        if (WaterLevelManager.Instance == null)
            return;
        Gizmos.DrawCube(
            new Vector3(transform.position.x, WaterLevelManager.Instance.waterLevel + GetWaterHeight(),
                transform.position.z), Vector3.one * .3f);
    }
